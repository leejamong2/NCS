...
public class Main_math {
	...
	System.out.println("\n=== 소수부 둘째자리까지 나타내면서 반올림 한 후 ===");
	System.out.println("db1 => "+ Math.round(db1*100)/100.0 + ", db2 =>" + Math.round(db2*100)/100.0);
	// db1 => 93.46, db2 =>86.88
	
	
	// 2. >> 올림에 대해서 알아본다.<<  ==> 웹에서 게시판 작성시 사용된다.
	//54.0 ==> 54.0  ==> 54    ////소수부가 .0일때만 0     소수부가 있을 땐 올리기
	//54.1 ==> 55.0  ==> 55
	//54.9 ==> 55.0  ==> 55
	
	double db3 = 54.0, db4=54.1, db5=54.9;
	System.out.println("\n=== 올림한 후 ===");
	System.out.println("db3=>"+Math.ceil(db3) + ",db4=>"+Math.ceil(db4) + "db5=>"+Math.ceil(db5));   ////세일은리턴타입도 더블타입
	// db3=>54.0,db4=>55.0db5=>55.0
	
	
	System.out.println("db3=>"+(int)Math.ceil(db3) + ",db4=>"+(int)Math.ceil(db4) + ",db5=>"+(int)Math.ceil(db5));  //소수부를 없애겠다
	// db3=>54, db4=>55, db5=>55       ////소수부가 .0일때만 0     소수부가 있을 땐 올리기
	
	
	// 3. >> 내림에 대해서 알아본다.<<  
	//54.0 ==> 54.0  ==> 54    ////소수부가 있든지 없든지 내려버림.  
	//54.1 ==> 54.0  ==> 55
	//54.9 ==> 54.0  ==> 55
	
	db3 = 54.0; db4=54.1; db5=54.9;
	System.out.println("\n=== 내림한 후 ===");
	System.out.println("db3=>"+Math.floor(db3) + ",db4=>"+Math.floor(db4) + ",db5=>"+Math.floor(db5));   ////리턴타입도 더블타입 = 실수로 바꿔준다
	// db3=>54.0, db4=>54.0 db5=>54.0
	
	
	System.out.println("db3=>"+(int)Math.floor(db3) + ",db4=>"+(int)Math.floor(db4) + ",db5=>"+(int)Math.floor(db5));  
	// db3=>54, db4=>54, db5=>5      ////소수부가 있든지 없든지 내려버림.  
	...
	int a = 20,  b = -30;
	System.out.println("a=>"+Math.abs(a)+", b=>"+Math.abs(b));  ////정수타입
	// a=>20, b=> -30
