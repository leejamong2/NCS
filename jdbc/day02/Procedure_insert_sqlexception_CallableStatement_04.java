...
/*
 	===HR에서 생성해두었던 프로시저 pcd_tbl_member_test1_insert 을 사용해본다
      ...
      '영업시간(월~금 14:00 ~ 16:59:59 까지) 아니므로 입력불가함!!' 이라는 오류메시지가 뜨도록 한다. 
         ...
         select to_char(sysdate, 'd')  -- sysdate 의 주의 일요일부터 (지금은 2024년 2월 18일) sysdate(지금은 2024년 2월 20일) 까지 며칠째 인지를 알려주는 것이다.
    ...
    begin 
          ...
          if( to_char(sysdate, 'd') in('1','7') or 
              ...
              raise error_dayTime;
          else --입력(insert)이 가능한 요일명과 시간대라면 암호를 검사하겠다.
          ...
          exception
             when error_dayTime then
                  raise_application_error(-20003, '>> 영업시간 (월~금 12:00 ~ 16:59:59 까지) 이 아니므로 입력불가함 <<');
	...
	public static void main(String[] args) {
		Connection conn = null;
	      ...
	      try {
		         ...
		         //        즉, 우편배달부(택배기사) 객체 만들기
		         cstmt = conn.prepareCall("{call pcd_tbl_member_test1_insert(?,?,?)}");//인모드가 세개라 ?위치홀더 셋
		         /*
		         ...
		         */
			      ...
			      cstmt.setString(1, userid);  // 숫자 1 은 프로시저 파라미터중 첫번째 파라미터인 IN 모드의 ? 를 말한다. 첫번째?에 유저아이디들어감
			      cstmt.setString(2, passwd);
			      cstmt.setString(3, name);
		         
			      // 4. CallableStatement cstmt 객체를 사용하여 오라클의 프로시저 실행하기
			      int n = cstmt.executeUpdate();  //오라클 서버 해당 프로시저를 실행하라는 말.  리턴타입 int. 
		         ...
		         //e.printStackTrace();
		    	  ...
		    	  if(e.getErrorCode() == 20002 || e.getErrorCode() == 20003) {
		    		  System.out.println(e.getMessage());  //e.getMessage() 가 aise_application_error(-20003, '>> 영업시간 (월~금 12:00 ~ 16:59:59 까지) 이 아니므로 입력불가함 <<'); 임
		    	  }
		    	  else if(e.getErrorCode() == 1) { //프라이머리 키니까 중복되믄 오류징
		    		  System.out.println(">>>아이디는"+userid+"현재 사용중이므로 다른 아이디로 입력하세요");
		    	  }
