...
create or replace procedure pcd_student_select_one
    ...
    begin
          ...
          if v_cnt = 0 then
            ...
            SELECT S.name, S.tel, S.addr, to_char(S.registerdate, 'yyyy-mm-dd hh24-mm-ss'),
                   C.classname, C.teachername
                   INTO
                   o_name, o_tel, o_addr, o_registerdate, o_classname, o_teachername --변수에 넣어줌(out)  --아웃모드는 타입을 정해줘야 나타낼수있따
            FROM
            (
            select *
            from tbl_student
