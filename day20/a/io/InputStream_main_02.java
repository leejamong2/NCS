...
*/      
	...
	public static void main(String[] args) throws IOException {
		...
		System.out.println(">> 영문자를 입력하세요 [종료하려면 x룰 압력허세요] <<");
		
		int input = 0;
		
		while( (input = System.in.read()) != -1 ) {
			// -1 은 키보드로 Ctrl + Z(강제종료)를 입력한 값이다.
			// 키보드로 Ctrl + Z(강제종료)를 하면 while 을 빠져 나간다.
			// System.in.read() 는 1byte 씩 읽어온다.
			
			System.out.println("\n === 키보드로 부터 데이터 입력받기 ==="); 
			System.out.println("화면에 출력 [println 을 사용한 input] : " + input); //숫자
			System.out.print("화면에 출력 [write 을 사용한 input] : "); 
			System.out.write(input); //문자
			System.out.flush();  
			/*
			...
			*/
			...
			System.out.println("");
			System.out.println("화면에 출력 [println 을 사용한 (char)input] : " + (char)input); //이렇게 하기 귀차나서 write로 씀
