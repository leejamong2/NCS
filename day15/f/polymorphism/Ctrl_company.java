		...
		//	또는
			...
			if(cnt == 0) {
				...
				for(int i=0; i<CommonMember.count; i++) {
					...
					if(	cmbr_arr[i] instanceof Gujikja &&
						input_gender.equals( ((Gujikja)cmbr_arr[i]).getGender()) ) { //커먼멤버에는 겟젠더가 없으니까 구직자로 바뀌어야함 그러려면 캐스트해야지((Gujikja)cmbr_arr[i]). 내가 쓴 성별과 일치하는 회원이 있다면
						isSearch = true;
				...
				for(int i=0; i<CommonMember.count; i++) {
					if(cmbr_arr[i] instanceof Gujikja) {
						if(Integer.parseInt(str_ageLine) == ((Gujikja)cmbr_arr[i]).getAge()/10*10
						   && input_gender.equals( ((Gujikja)cmbr_arr[i]).getGender()) ) { //내가 쓴 연령대와 성별 일치하는 회원이 있다면
