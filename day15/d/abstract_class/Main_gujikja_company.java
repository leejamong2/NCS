...
public class Main_gujikja_company {
	...
	// 	오류!! 미완성클래스(추상클래스)는 객체화(인스턴스화) 할 수 없기 떄문이다.
		...
		do {//4번 할때까지 돌거야
			...
			switch (str_menuno) {
			case "1":   //구직자회원가입
				ctrl_gu.register(sc, gu_arr);  //(sc, gu_arr) gu_arr에 저장해야해서 
				break;

			case "2":   //구인회사 회원가입
				ctrl_cp.register(sc, cp_arr);// 회원저장소에서 꺼내올거니까 gu_arr		
				break;
			...
			case "3":   //구직자 로그인
				...
				if(login_gu != null) {
					...
					ctrl_gu.gu_menu(sc, login_gu, cp_arr, rc_arr, rcApply_arr);
				}
				else {
			...
			case "4" :	//구인회사 로그인
				...
				if(login_cp != null) {
					...
					ctrl_cp.cp_menu(sc, login_cp, gu_arr, rc_arr, rcApply_arr);  //구인회사 전용메뉴 , 모집공고저장소
				}
				else {
