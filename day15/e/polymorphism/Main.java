...
public class Main {
	...
	// 자식클래스로 생성되어진 객체를 부모 클래스 타입으로 받을 수 있다는 것이 다형성(Polymorphism)이다. (암깅!!!!!!!!!!!!)
	
	public static void main(String[] args) {
		
	//	Animal an = new Animal();  
	//	오류!!!  abstract class 를 사용해서 객체를 생성할 수 없다 
		
		Animal[] ani_arr = new Animal[5];  //공간만 만든 것@@@@@
	//	미완성 클래스는 스스로 객체생성(인스턴스)은 불가하지만 
	// 	일반적인 자식클래스로 생성된 객체를 저장하는 용도로 사용이 가능하다.!!!
		
		
		Dog dog = new Dog();  // 얘가 객체 생성한것@@@@@
	...
	//	ani_arr[2] = new Duck(); // 가능 ==> 다형성
		...
		for(int i=0; i<ani_arr.length; i++) { //다섯번 반복
			if(ani_arr[i] != null) {
			ani_arr[i].view_info();  
			}
		}//end of for-----------------------------
