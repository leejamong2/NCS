...
public class Ctrl_company extends Ctrl_common{
	...
	public void register(Scanner sc, Company[] cp_arr) {
		...
		if(Company.count < cp_arr.length) { //지금까지 생성된 구인회사가 cp_arr.length(배열길이==>정원) 보다 적을 경우에만 신규회원을 받아들인다.
			...
			do {
				...
				for(int i=0; i<Company.count; i++) {
					...
					if(id.equals(cp_arr[i].getId()) ) {  //내가 입력한 아이디와 저장된 아이디가 같냐
...
*/			
			...
			System.out.println(">>정원 " + cp_arr.length + "개가 꽉 차서 구인회사 회원가입이 불가합니다.<< \n");
		}
	...
	//우리 회사 정보보기
	void view_myInfo(Company login_cp) {
	...
	// 우리 회사 정보 수정하는 메소드 생성하기 //
		 void update_myInfo(Scanner sc, Company login_cp) {
	...
	// == 우리회사 채용공고 조회 ==
		private void view_recruit_mycompany(Company login_cp, Recruit[] rc_arr) {
			...
			for(int i=0; i<Recruit.count; i++) {
				...
				if(login_cp.getId().equals(rc_arr[i].getCp().getId()) ) { //로그인되어진 회사아이디와 모집공고를낸 회사 아이디가 같냐
					is_existence = true;
