...
import my.day13_14.a.inheritance.Recruit;
...
public class Ctrl_gujikja_imple extends Ctrl_common_imple implements Ctrl_gujikja{
	...
	@Override
	public void register(Scanner sc, CommonMember[] cmbr_arr) { //레지스터는 회원가입 시켜주는 메소드
		
		if(CommonMember.count < cmbr_arr.length) { //지금까지 생성된 구직자 회원수가 cmbr_arr.length(배열길이==>정원) 보다 적을 경우에만 신규회원을 받아들인다.
			...
			do {
				...
				for(int i=0; i<CommonMember.count; i++) {
					...
					if(cmbr_arr[i].getType()==1 && id.equals(cmbr_arr[i].getId())) {  //저장된 것중에서 타입이 1인 애를 꺼내와야하니까. 내가 입력한 아이디와 저장된 아이디가 같냐
...
*/			
			...
			System.out.println(">>정원 " + cmbr_arr.length + "명이 꽉 차서 구직자 회원가입이 불가합니다.<< \n");
		}
		...
		// === 구직자 전용메뉴 메소드 생성하기 === //
		@Override
		public void gu_menu(Scanner sc, Gujikja_imple login_gu,  CommonMember[] cmbr_arr, my.day16.d.INTERFACE.Recruit[] rc_arr, RecruitApply[] rcApply_arr) {
			String str_menuno;
			do {
				...
				case "2":  //내 정보 수정
					...
					break;
				case "3":  //모든구인회사 조회
					view_company_info(cmbr_arr); //메소드의 오버로딩. 파라미터 다름
					break;
				
				case "4": //구인회사검색하기
					view_company_info(sc, cmbr_arr); //오버로딩. 파라미터 다름
					break;
		...
		// 내 정보 보기//
		private void view_myInfo(Gujikja_imple login_gu) {  //접근제한자가 프라이빗으로 되어있어서 다른 클래스에서 호출 못함. 구직자전용메뉴 gu_menu를 통해서만 볼 수 있다.
		...
		private void view_company_info(Scanner sc, CommonMember[] cmbr_arr) {
				...
				do {
					...
					case "1":  //업종검색
						...
						break;
					case "2":  //자본금검색
						search_seedmoney_company(sc, cmbr_arr);  //리턴타입 모써야할까?  보이드.  보여주면 끝나니까
						break;
					case "3":  //구직자메뉴로 돌아가기
		...
		private void search_jobtype_company(Scanner sc, CommonMember[] cmbr_arr) {
			...
			for(int i=0; i<CommonMember.count; i++) {
				...
				//cmbr_arr[i] instanceof Company 는  cmbr_arr[i] 객체가 Company
					is_existence = true;
					sb.append(cmbr_arr[i].getInfo()+"\n");
				}
   ...
   //모든채용공고조회  (채용마감일자가 오늘이 지난 것은 보여주면 안된다)
   private void view_all_recruit_info(my.day16.d.INTERFACE.Recruit[] rc_arr) {
	   if(Recruit_imple.count == 0) {
		   ...
		   try {
			   ...
			   for(int i=0; i<Recruit_imple.count; i++) {
				   ...
				   if(!date_finish_day.before(today)) {  //!채용마감일자.before(오늘날짜)  ==> 채용마감일자가 오늘이 지나지 않은경우
					   sb.append(rc.getRecruit_no()+"\t\t"+
							   	 rc.getCp().getName()+"\t"+ //리크루트에서 컴패니필드를 꺼내고 그 컴패니에서 상속받은 네임필드를 가져옴.
   	...
   	//채용응모하기
	private void input_recruitApply(Scanner sc, Gujikja_imple login_gu, my.day16.d.INTERFACE.Recruit[] rc_arr, RecruitApply[] rcApply_arr) {
		...
		if(str_my_recruit_no.length()>0) { //응모한경우가 있는 경우라면
		...
		do {
			...
			for(int i=0; i<Recruit_imple.count; i++) {
				rc = (Recruit_imple) rc_arr[i];
				if(input_recruit_no.equals(String.valueOf(rc.getRecruit_no())) ) {
					is_existance = true;
					  //채용공고에 해당하는 번호를 넣어준다
					break;
				}
