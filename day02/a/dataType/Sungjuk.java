...
public class Sungjuk {
	...
	*/
		...
		//	int total = kor + eng + math; // 0 ~ 300
			...
			double avg4 = (double)total/3;   // 정수형 타입인 total을 강제로 double타입으로 형변환 시키는 것이다. 강제형변환(casting)
			float avg5 = (float)total/3;     // 정수형 타입인 total을 강제로 float타입으로 형변환 시키는 것이다. 강제형변환(casting)
			...
			if( avg3 >= 90 ) {
		        hakjum = 'A';        //...글자 하나만 가능. 한글도 가능. ㄱ도 가능.				
			}
		...
		static void char_print() {
			...
			// ...
			// Z는 10진수로 90로 보자. z는 10진수로 122로 보자.
			...
			System.out.println(ch1+0);  // 65  //...int로 바뀜
		...
		/*	
			...
			System.out.println(""+ch4+ch5+ch6);  // 이지윤
		*/	
		}
